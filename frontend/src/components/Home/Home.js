    ...
    const fetchFeaturedBooks = async () => {
      try {
        const response = await fetch(
          "https://www.googleapis.com/books/v1/volumes?q=the"
        );
    ...
    <div className="homepage-container">
      <div className="intro-section">
        ...
        <p className="intro-text">
          Dive into a world of endless stories and adventures. Explore,
          discover, and read your favorite books for free!
        </p>
      </div>
      ...
      <div className="featured-section">
        ...
        <div className="featured-books-list">
          {featuredBooks.map((book, index) => (
            <Card key={index} book={book} onClick={redirectToBrowse} />
          ))}
        </div>
      </div>
    </div>
